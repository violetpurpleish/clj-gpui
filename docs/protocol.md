...
The optional root-window `on-missing-glyphs` callback also remains protocol v11. It uses the existing callback transport with an array of `{"grapheme": "…", "font-class": "proportional" | "monospace"}` reports. Omission/null disables collection; older hosts ignore the field. See [font diagnostics](font-diagnostics.md) for delivery and platform support (GPUI 0.3.6 emits reports on Linux, not macOS or Windows DirectWrite).
...
| `on-missing-glyphs` | string callback id or null | Root `window` only: opt-in application-wide font diagnostics; receives an array of `grapheme` / `font-class` maps. Reports resolve the current handler and are discarded when disabled. See [platform support](font-diagnostics.md) |
...
| `follow-child` | string | `message-scroller`: keep a row or descendant's laid-out top at the viewport top using GPUI autoscroll. Use a paragraph's row ID to follow whole paragraphs, or a word ID to follow wrapped lines. Pair with `scroll-to-item` and `scroll-generation` to realize the target row on changes/seeks. A virtual trailing spacer permits final-row alignment. Wrapping is measured natively. Use `list-style` padding 0 for exact top alignment. Omitted / nil disables following; `scroll-to-end true` takes precedence. |
| `follow-resume-delay` | number | `message-scroller`: seconds to suspend following after manual wheel/trackpad scrolling. Each event restarts the delay. Pointer interaction holds suspension until release plus the delay. Both descendant alignment and playback-driven item jumps are suppressed while paused; highlighting can still update. A native timer resumes at the latest target even without a new Clojure tree. Omitted / invalid leaves following locked. |
| `follow-generation` | number or string | `message-scroller`: changing this explicit token cancels the suspension and resumes following immediately. Keep separate from the playback-driven `scroll-generation`. Removing the scroller cancels its timer. |
...
Chat `message` / `bubble` / `attachment` / `marker` are Kit `RenderOnce` primitives on the wire (same completeness rule as `table`). A `bubble` child of `message-content` uses Kit `.bubble` so Ghost still strips header/footer inset. Message `:footer` is a `message-footer` child, not the sheet `footer` field. Direct children after an explicit `bubble-content` append through Kit `ParentElement` so the content slot's `StyleRefinement` is kept. `attachment-media` applies `with_size` only when `control-size` is set (omitted inherits the parent Attachment size). Ordinary media children are `.child`; Kit `.overlay` is `attachment-media-overlay`. Nested style maps cover `with_stack_style`, `ShimmerStyle`, `separator_style`, and MessageScroller `with_*_style` / `with_jump_button_renderer`. Chat nodes use the same visual/layout style vocabulary as ordinary widgets. MessageScroller root `Styled` applies to the Kit widget; the host wrapper keeps viewport/box geometry. `jump-button-label` is the jump button tooltip; renderer `text` (Clojure `:label`) is Kit `Button::label`. `message-scroller` keeps host `MessageScrollerState` (tail follow on). Row identity is `id` or `idx:{n}`; prepend/append without `reset` needs a stable `id`. Rows and comparable layout snapshots update only when a new Clojure tree arrives; native scroll frames reuse them. Layout comparisons ignore generated callback IDs and text colors, so playback highlighting preserves measured heights. Changed rows remeasure individually; append/prepend also remeasures when a surviving row changed. Scroller rows are the static overlay subset plus this chat family. Clickable labels with a stable `id` queue their identity within the owning scroller and resolve `on-click` against the latest installed tree after pending renders and earlier callbacks finish. This keeps transcript word clicks valid during playback updates; a click is discarded if its scroller or label disappears. Give each transcript a distinct scroller `id`, and each word a unique `id` within it. `:scroll-to-item` / `:scroll-to-end` are Kit `scroll_to_item` / `scroll_to_end` (programmatic; omitted leaves native scroll; `:scroll-generation` re-applies the same target). An unresolved or rejected `:scroll-to-item` is not recorded as applied, so the same request can succeed after append/load. Remaining: Kit's arbitrary row renderer (`IntoElement` / stateful nodes).
