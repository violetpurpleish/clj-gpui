...
| `follow-generation` | number or string | `message-scroller`: changing this explicit token cancels the suspension and resumes following immediately. Keep separate from the per-word `scroll-generation`. Removing the scroller cancels its timer. |
